...
void pickup(Player *player, Room *current_room, char *item) {
    if (strcmp(current_room->item, item) == 0) {
        if (strcmp(item, "Healthapple") == 0) {
            ...
            int gold_amount = 50;  // Or any amount you prefer
            player->gold += gold_amount;
            printf("You picked up %d gold. You now have %d gold.\n", gold_amount, player->gold);
        } else {
...
void load_game(Player *player, Room **current_room, const char *filepath) {
    ...
    for (int i = 0; i < NUM_ROOMS; ++i) {
        ...
        rooms[i]->up = (Room *)(intptr_t)up_id;
        rooms[i]->down = (Room *)(intptr_t)down_id;
        rooms[i]->left = (Room *)(intptr_t)left_id;
        rooms[i]->right = (Room *)(intptr_t)right_id;
    }
    ...
    for (int i = 0; i < NUM_ROOMS; ++i) {
        ...
        int up_id = (int)(intptr_t)room->up;
        int down_id = (int)(intptr_t)room->down;
        int left_id = (int)(intptr_t)room->left;
        int right_id = (int)(intptr_t)room->right;
